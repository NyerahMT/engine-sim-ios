    ...
    const RuntimePaths &runtimePaths)
{
    ...
    m_assetPath =
        runtimePaths.assetDirectory.string();
...
void EngineSimApplication::renderScene()
{
    ...
    renderDiagnosticLog(
        "screen=%d dimensions=%dx%d",
        m_screen,
        m_screenWidth,
        m_screenHeight);

    renderDiagnosticLog(
        "renderer=%p engineView=%p",
        static_cast<void *>(
            m_renderer),
        static_cast<void *>(
            m_engineView));
    ...
    renderDiagnosticLog(
        "rightGauge=%p osc=%p performance=%p load=%p mixer=%p info=%p",
        static_cast<void *>(
            m_rightGaugeCluster),
        static_cast<void *>(
            m_oscCluster),
    ...
    renderDiagnosticLog(
        "geometryGenerator.reset BEGIN");

    m_geometryGenerator.reset();

    renderDiagnosticLog(
        "geometryGenerator.reset END");
        ...
        != nullptr)
    {
        ...
        renderDiagnosticLog(
            "windowBounds created");

        const bool dashboardReady =
            m_rightGaugeCluster != nullptr
            && m_oscCluster != nullptr
            && m_performanceCluster != nullptr
            && m_loadSimulationCluster != nullptr
        ...
        {
            /*
             * Do not let a partially initialized dashboard crash the
             * entire renderer. Give us a usable engine view and record
             * which pointer is missing in engine-sim.log.
             */

            renderDiagnosticLog(
                "WARNING: dashboard incomplete; using engine-only fallback");
        ...
        {
            renderDiagnosticLog(
                "screen 0 dashboard layout BEGIN");
            ...
            renderDiagnosticLog(
                "engineView bounds END");

            renderDiagnosticLog(
                "rightGauge bounds BEGIN");
        ...
        renderDiagnosticLog(
            "getCameraPosition BEGIN");

        const Point cameraPosition =
            m_engineView->getCameraPosition();

        renderDiagnosticLog(
            "getCameraPosition END x=%.6f y=%.6f",
            static_cast<double>(
                cameraPosition.x),
            static_cast<double>(
                cameraPosition.y));
        ...
        renderDiagnosticLog(
            "camera LoadVector END");

        renderDiagnosticLog(
            "CalculateUiCamera BEGIN");
        ...
        renderDiagnosticLog(
            "engineView geometry w=%.6f h=%.6f zoom=%.6f",
            static_cast<double>(
                boundsWidth),
            static_cast<double>(
                boundsHeight),
        ...
        {
            renderDiagnosticLog(
                "ERROR: invalid bounds/zoom; skipping render content");

            renderDiagnosticLog(
                "uploadGeometry fallback BEGIN");
            ...
            renderDiagnosticLog(
                "uploadGeometry fallback END");

            renderDiagnosticLog(
                "endFrame fallback BEGIN");

            m_renderer->endFrame();

            renderDiagnosticLog(
                "endFrame fallback END");
        ...
        m_renderer->setSceneViewport(
            ...
            static_cast<float>(
                m_screenHeight)
                - m_engineView->m_bounds.top(),

            m_engineView->m_bounds.width(),
            m_engineView->m_bounds.height());

        renderDiagnosticLog(
            "setSceneViewport END");

        renderDiagnosticLog(
            "RENDER CONTENT BEGIN objects=%llu",
            static_cast<unsigned long long>(
                m_objects.size()));

        /*
         * Split render() into its three major stages here so we can see
         * whether the failure is geometry generation, simulation-object
         * drawing, or UI drawing.
         */

        renderDiagnosticLog(
            "object generateGeometry BEGIN");
        ...
        {
            renderDiagnosticLog(
                "generateGeometry object=%llu ptr=%p BEGIN",
                static_cast<unsigned long long>(
                    objectIndex),
                static_cast<void *>(
                    object));
            ...
            renderDiagnosticLog(
                "generateGeometry object=%llu END",
                static_cast<unsigned long long>(
                    objectIndex));
            ...
            ++sublayer)
        {
            ...
            renderDiagnosticLog(
                "object render sublayer=%d BEGIN",
                sublayer);
            ...
            {
                renderDiagnosticLog(
                    "object render sublayer=%d object=%llu ptr=%p BEGIN",
                    sublayer,
                    static_cast<unsigned long long>(
                        objectIndex),
                    static_cast<void *>(
                ...
                renderDiagnosticLog(
                    "object render sublayer=%d object=%llu END",
                    sublayer,
                    static_cast<unsigned long long>(
                        objectIndex));
            ...
            renderDiagnosticLog(
                "object render sublayer=%d END",
                sublayer);
        }

        renderDiagnosticLog(
            "UI manager render BEGIN");

        m_uiManager.render();

        renderDiagnosticLog(
            "UI manager render END");
    ...
    {
        renderDiagnosticLog(
            "engineView NULL; render content skipped");
    }
    ...
    renderDiagnosticLog(
        "geometry counts vertices=%d indices=%d",
        vertexCount,
        indexCount);

    renderDiagnosticLog(
        "uploadGeometry BEGIN");
    ...
    Shaders::StageEnableFlags flags)
{
    ...
    ysMatrix transform =
        m_shaders.m_objectVariables.Transform;
...
void EngineSimApplication::refreshUserInterface()
{
    ...
    m_engineView =
        m_uiManager
            .getRoot()
            ->addElement<EngineView>();

    m_rightGaugeCluster =
        m_uiManager
            .getRoot()
            ->addElement<RightGaugeCluster>();

    m_oscCluster =
        m_uiManager
            .getRoot()
            ->addElement<OscilloscopeCluster>();

    m_performanceCluster =
        m_uiManager
            .getRoot()
            ->addElement<PerformanceCluster>();

    m_loadSimulationCluster =
        m_uiManager
            .getRoot()
            ->addElement<LoadSimulationCluster>();

    m_mixerCluster =
        m_uiManager
            .getRoot()
            ->addElement<MixerCluster>();

    m_infoCluster =
        m_uiManager
            .getRoot()
            ->addElement<InfoCluster>();

    m_infoCluster->setEngine(
        m_iceEngine);

    m_rightGaugeCluster->m_simulator =
        m_simulator;

    m_rightGaugeCluster->setEngine(
        m_iceEngine);

    m_oscCluster->setSimulator(
        m_simulator);

    m_performanceCluster->setSimulator(
        m_simulator);

    m_loadSimulationCluster->setSimulator(
        m_simulator);

    m_mixerCluster->setSimulator(
        m_simulator);
}
