...
{
    m_crankshaftCount =
        params.crankshaftCount;

    m_cylinderCount =
        params.cylinderCount;

    m_cylinderBankCount =
        params.cylinderBanks;

    m_exhaustSystemCount =
        params.exhaustSystemCount;

    m_intakeCount =
        params.intakeCount;

    m_starterTorque =
        params.starterTorque;
    ...
    m_initialHighFrequencyGain =
        params.initialHighFrequencyGain;

    m_initialSimulationFrequency =
        params.initialSimulationFrequency;

    m_initialJitter =
        params.initialJitter;
...
Crankshaft *
Engine::getOutputCrankshaft() const
{
    return
        &m_crankshafts[0];
}

void Engine::setSpeedControl(
    double s)
{
    m_throttle
        ->setSpeedControl(
...
double
Engine::getThrottlePlateAngle() const
{
    return
        (
            1
            - m_intakes[0]
                .getThrottlePlatePosition()
        )
        * (
            constants::pi
            / 2
...
double
Engine::getTotalVolumeFuelConsumed() const
{
    return
        getTotalFuelMassConsumed()
        / m_fuel
            .getDensity();
}
